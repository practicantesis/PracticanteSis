...
$ex=GetUserInfoFromLDAP($_POST['user'],"exists");
$dn = "cn=".$_POST['dn'].",ou=Corporativo,ou=Samba,ou=groups,dc=transportespitic,dc=com";

if($ex == 0) {
    $data = "Usuario NO existe en usuarios LDAP";
    $err = "Usuario NO existe en usuarios LDAP";
    $success = "NO";
} else {
    if ($_POST['accion'] == "add" ) {
        $err=AppendLDAPVAl($dn,$_POST['user'],'memberuid');
        $data = "Usuario existe en LDAP";
        $success = "YES";
...
if ($_POST['accion'] == "delete" ) {
    #echo "PERRO!!!";
    $err=DeleteLDAPVAl($dn,$_POST['valor'],'memberuid');
    $data = "Usuario existe en LDAP";
    $success = "YES";
}
...
$list=array();
if ($_POST['tipo'] == "Porxxxxuse") {
    $list=smbGetGroupsForUser($_POST['group'],"array");    
    //print_r($list);
    $cons=0;
...
if ($_POST['tipo'] == "Pornom") {
    //$list=smbGetGroupMembers($_POST['group'],"array","groupname");    
    $listm=smbGetGroupMembers($_POST['group'],"array","smbgroupmembers");
    /*   
    //print_r($list);
    $cn=1;
...
$listm.='    xxxxxxxxxxxxxxxxxx        <div class="dropdown-menu">
              <a class="dropdown-item" href="#" onClick="AddUserToGroup('."'".$valor."'".','."'".$_POST['group']."'".','."'".$cn."'".');">Agregar Usuario al Grupo</a>
            </div>
          </div> <br><div class="card"><div class="card-body"><div id="CapturaNUser'.$cn.'"></div></div></div><br>';
