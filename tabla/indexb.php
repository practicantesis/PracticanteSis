...
<script type="text/javascript">
	...
	function busquedaPorOfficina() {
		...
		var optionSelect = select.options[select.selectedIndex]; //obtenemos las obciones que hay dentro del select
		...
		for (var i = 1; i < tabla.rows.length; i++) {
			...
			for (var j = 0; j < cellsOfRow.length && !found; j++) {
				...
				//si encuntra coincidencia
				if (cellsOfRow[j].innerHTML === Pbusqueda) {
					found = true;

				} //si la obcion esta vacia en la busqueda found es true para que muestre todo el recorrdio
				else if (Pbusqueda == "") {
					found = true;
				}
			}
...
<body>
	...
	<center>
		...
		if ($con) {
			...
			$arr=GetDevUsersFromLDAPCells("array",$info[$i]['duoficina'][0],$con);
			echo '<table id="datos" class="table table-hover">';
			echo '<thead class="encabezado2"><tr><th>Nombre</th><th>Numero de empleado</th><th>Oficina</th><th>Usuario</th><th>Tag</th><th>Informacion</th></tr></thead>';
